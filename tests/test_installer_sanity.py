...
def test_ps1_setup_references_quarto():
    ...
    ps1 = (ROOT / "packaging" / "setup_dependencies.ps1").read_text(encoding="utf-8")
    assert "quarto" in ps1.lower(), (
        "setup_dependencies.ps1 does not mention quarto"
    )
...
def test_ps1_setup_references_tinytex():
    ...
    ps1 = (ROOT / "packaging" / "setup_dependencies.ps1").read_text(encoding="utf-8")
    assert "tinytex" in ps1.lower(), (
        "setup_dependencies.ps1 does not mention tinytex"
    )
...
def test_launch_setup_ps1_exists_and_ascii():
    ...
    assert not non_ascii, (
        f"launch_setup.ps1 has {len(non_ascii)} non-ASCII char(s)"
    )
